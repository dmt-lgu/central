...
interface MonthlyData {
  BP?: Array<{ date: string; values: LguDetails[] }>;
  CO?: Array<{ date: string; values: LguDetails[] }>;
  WP?: Array<{ date: string; values: LguDetails[] }>;
  BC?: Array<{ date: string; values: LguDetails[] }>;
}
...
function App() {
  ...
  useEffect(() => {
    if (dates && dates.length > 0 && monthlyStats) {
      const filtered = filterMonthlyStatsByDates(monthlyStats, dates, regions);
      dispatch(setData(filtered));
      console.log('Filtered Monthly Stats:', filtered);
    }
  }, [monthlyStats, dates, regions]);

  useEffect(() => {
    setMonthlyStats({});
    if (services.includes('Building Permit')) getBP();
    if (services.includes('Certificate of Occupancy')) getCO();
    if (services.includes('Working Permit')) getWP();
    if (services.includes('Barangay Clearance')) getBC();
  ...
  const filterMonthlyStatsByDates = (stats: MonthlyData, selectedDates: string[], selectedRegions: string[]) => {
    if (!selectedDates?.length || !stats) return stats;
          ...
          monthData.values.forEach((item: any) => {
            if (selectedRegions.includes(item.dictRo)) {
              if (!regionCounts[item.dictRo]) {
                regionCounts[item.dictRo] = { operational: 0, developmental: 0, training: 0, withdraw: 0 };
              }
              switch (item.universalStatus) {
                case '[A] Operational': regionCounts[item.dictRo].operational++; break;
                case '[B] Developmental': regionCounts[item.dictRo].developmental++; break;
                case '[C] For Training/Others': regionCounts[item.dictRo].training++; break;
                case '[D] Withdraw': regionCounts[item.dictRo].withdraw++; break;
              }
            }
          });
          return { date: monthData.date, data: Object.entries(regionCounts).map(([region, counts]: any) => ({ region, ...counts })) };
        })
  ...
  const fetchData = (url: string, key: keyof MonthlyData) => {
    axios.get(url, { headers: { Authorization: `Token ${import.meta.env.VITE_TOKEN}` } })
      .then((response) => {
        const data = response.data.values;
        const records = data.slice(3);
        const groupedByMonth = records.reduce((acc: any, row: any) => {
          const period = row[1];
          const lguDetails: LguDetails = {
            id: row[0], lguName: row[4], status: row[5], progressRate: row[6], concerns: row[7],
            actionItems: row[8], otherRemarks: row[9], universalStatus: row[10], mpar: row[11],
            geocode: row[12], name: row[13], province: row[14], region: row[15], district: row[16],
            level: row[17], incomeClass: row[18], dictRo: row[19]
          };
          if (!acc[period]) acc[period] = { date: period, values: [] };
          acc[period].values.push(lguDetails);
          return acc;
        }, {});
        const result: any = Object.values(groupedByMonth);
        setMonthlyStats(prevStats => ({ ...prevStats, [key]: result }));
      });
  };

  const getBP = () => fetchData('18kaPQlN0_kA9i7YAD-DftbdVPZX35Qf33sVMkw_TcWc/values/BP1 UR Input', 'BP');
  const getCO = () => fetchData('18kaPQlN0_kA9i7YAD-DftbdVPZX35Qf33sVMkw_TcWc/values/BPCO UR Input', 'CO');
  const getWP = () => fetchData('18kaPQlN0_kA9i7YAD-DftbdVPZX35Qf33sVMkw_TcWc/values/WP UR Input', 'WP');
  const getBC = () => fetchData('18kaPQlN0_kA9i7YAD-DftbdVPZX35Qf33sVMkw_TcWc/values/BC UR Input', 'BC');
          ...
          <div className="w-[90%] flex flex-col gap-10 min-h-[10px]">
            ...
            <div className="flex-1 ml-[20vw] lg:ml-0">
              ...
              {showScroll && (
                <button
                  onClick={scrollToTop}
                  className="fixed bottom-20 right-6 p-3 bg-blue-500 text-white rounded-full shadow-lg hover:bg-blue-700 transition"
                >
                  <ChevronUp size={24} />
                </button>
              )}
